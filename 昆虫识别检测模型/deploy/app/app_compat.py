...
def create_app(init_flag: bool = False, configs: dict = None):
    ...
    app_instance = Flask(__name__,static_url_path='',static_folder=appPath+r'static')
    ...
    def index():
        ...
        Provide index page.
        '''
        return send_file(appPath+"static/index.html")
    ...
    @app_instance.route("/get_test_image", methods=['GET', 'POST'])
    def get_test_image():
        return send_file(appPath+"static/test.jpg")

    @app_instance.route("/put_base64", methods=['GET', 'POST'])
    def put_base64():
        ...
        result = getResult(data, float(threshold), float(nms_threshold))
        return json.dumps(result)

    def getResult(image, threshold=0.5, nms_threshold=0.1):
        module_name="kunchongjiance"
        ...
        # print(results)
        # print(type(results))
        res = getModelResult.postprocess(results[0], threshold, nms_threshold)
        # print(res)
        # 打印预测结果
        return res
